...
public:
    ...
    void comsum(vector<int> &curr, int target, int sum, vector<int> &candidates, int curInd, int n){
        ...
        for(int i = curInd; i < n; i++){
            if(i != curInd && candidates[i]==candidates[i-1]) continue;  //to avoid picking up the same combnations i.e. we don't pick same element for certain kth position of a combination 
            sum += candidates[i];
